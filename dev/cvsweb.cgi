...
%REDIR_PATH = (
	'/cvsroot/apps/gated', '/kame-confidential/cvsweb.cgi',
	...
	'/cvsroot/doc/scopedaddr-format', '/kame-confidential/cvsweb.cgi',
...
#	'/cvsroot/viomagic', '/kame-confidential/cvsweb.cgi',
...
#	    'mozilla', '/a/mozilla-cvs',
		'kame-freebsd', '/cvsroot/kame-freebsd',
		'kame-freebsd-30', '/cvsroot/kame-freebsd-30',
		'kame-netbsd', '/cvsroot/kame-netbsd',
		'kame-bsdi', '/cvsroot/kame-bsdi',
		'mozilla6', '/cvsroot/mozilla6',
		'apps', '/cvsroot/apps',
		'apps-local', '/cvsroot/apps-local',
		'doc', '/cvsroot/doc',
		'unified-ipv6', '/cvsroot/unified-ipv6',
		'netbsd-sh3', '/cvsroot/netbsd-sh3',
		'netbsd', '/export0/ftp/.mirror1/ftp.netbsd.org/NetBSD-cvs/main',
#		'viomagic', '/cvsroot/viomagic',
		'kame', '/cvsroot/kame',
		'kame-local', '/cvsroot/kame-local',
		'apps-obsoleted', '/cvsroot/apps-obsoleted',
	    );
...
#	    'mozilla', 'Mozilla FreeBSD',
		'kame-freebsd', '#kame-FreeBSD',
		'kame-freebsd-30', '#kame-FreeBSD30',
		'kame-netbsd', '#kame-NetBSD',
		'kame-bsdi', '#kame-BSDI',
		'mozilla6', '#mozilla6',
		'apps', 'apps',
		'apps-local', 'apps-local',
		'doc', 'doc',
		'unified-ipv6', '#unified-ipv6',
		'netbsd-sh3', 'netbsd-sh3',
		'netbsd', 'netbsd',
#		'viomagic', 'viomagic',
		'kame', 'kame',
		'kame-local', 'kame-local',
		'apps-obsoleted', '#apps-obsoleted',
	    );
...
$intro = "
This is a WWW interface to the KAME CVS tree.
...
if ($target = (grep($fullname =~ /^$_/, keys %REDIR_PATH))[0]) {
	$fullname =~ /^$cvsroot(.*)/;
	$target = "http://$ENV{'SERVER_NAME'}$REDIR_PATH{$target}$1";
	$target .= '/' if ($target !~ /\/$/);
	$target .= $query;
	print <<EOF;
...

{
    ...
    if ($#foo > 1) {
	$intro .= "<p>\nThe following trees are obsoleted mainly because they have been merged in each original distribution:\n";
	for($i = 0; $i <= $#foo; $i++) {
	    next unless ($CVSROOTdescr{$foo[$i]} =~ s/^#//);
	    $intro .= qq{<a href="$scriptname?cvsroot=$foo[$i]">} .
		($CVSROOTdescr{$foo[$i]} ? 
		 $CVSROOTdescr{$foo[$i]} : $foo[$i]) . qq{</a>} .
		     ($i == $#foo  ? ".\n" : ",\n");
...
if (-d $fullname) {
	...
	closedir(DIR);
	if ($where eq '') {
	    print &html_header("KAME CVS Repository");
	    print $intro;
	} else {
